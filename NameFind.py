...
auth = firebase.auth()
user = auth.sign_in_with_email_and_password('diego.100pp@gmail.com', 123456)
...
def ID2Name(ID, conf):
    if ID>=1 and ID<=last_string:
        NameString = "Cliente: " + Names[ID-1] + " Confidence: " + (str(round(conf)) )                          # Find the Name using the index of the ID
        time.sleep(10)
        n = Names[ID-1]
        acesso = db.child('devloide/clientes').child(n).child('usuario').get(user['idToken'])
        code = db.child('devloide/clientes').child(n).child('senha').get(user['idToken'])
        if(n == acesso.val()):
            move = suporte.funcao_menu(n,code.val())
        else:
            NameString = " Cliente nao identificado de jeito nenhum"
            time.sleep(10)
            erro = suporte.error()


    else:
        NameString = " Cliente nao identificado "  # Find the Name using the index of the ID
        erro = suporte.error()
    return NameString
